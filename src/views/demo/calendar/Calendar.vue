  ...
  <div class="main">
    <div class="calender">
      <div class="left">
        <div class="topBar">
          <el-date-picker v-model="year" type="year" placeholder="年份"
                          size="default" style="width: 96px" value-format="YYYY"></el-date-picker>
          <el-select v-model="month" placeholder="月份" size="default" class="one">
            <el-option
          ...
          <el-button size="default" class="back" @click="back">返回今天</el-button>
        </div>
        <table class="dateWrap">
          ...
          <tr v-for="(item, index) in dayArray" :key="index">
            <td v-for="(day, key) in item" :key="key">
              <div :class="{'prevMonth': !day.cur, 'today': day.fullText === today, 'active': day.isSelect }" @click="changeDate(day)">
                <span>{{ day.text }}</span>
              </div>
            </td>
          </tr>
      ...
      <div class="right">
        ...
        <div class="lunar">
          <p>{{ dateInfo.lunar }}</p>
          <p>{{ dateInfo.lunarYear }} &nbsp; {{ dateInfo.animalsYear }}</p>
        </div>
        <ul class="holidayList">
          <li v-for="(item,index) in dateInfo.history" :key="index">{{ item.title }}</li>
        </ul>
        <div class="suit">
          <div class="default">
            <dl>
              <dt>宜</dt>
              <dd v-for="(item, index) in dateInfo.suit" :key="index">{{ item }}</dd>
            </dl>
            <dl>
              <dt>忌</dt>
              <dd v-for="(item, index) in dateInfo.avoid" :key="index">{{ item }}</dd>
            </dl>
          </div>
        </div>
      </div>
                    ...
                    if (i === 0) {
                        if (j < first) {
                            temp.unshift({
                                ...
                                cur: false,
                                text: prevMothDay,
                                fullText: `${getLastDateString(Number(date.year), Number(date.month), Number(prevMothDay))}`
                            })
                        ...
                        if (t > nums) {
                            temp.push({
                                ...
                                cur: false,
                                text: nextMonthDay,
                                fullText: `${getNextDateString(Number(date.year), Number(date.month), Number(nextMonthDay))}`
                            })
